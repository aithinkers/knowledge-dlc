...
test("FEAT-006 one engine produces equivalent direct, MCP, and generated-adapter outcomes", async (t) => {
  ...
  const server = new McpProjectServer({ root, projectId: "fixture.project", engineFactory, capabilities: ["kb_search","kb_fetch"] });
  ...
  const templates = await server.request({ jsonrpc: "2.0", id: 2, method: "resources/templates/list" }); assert.deepEqual(templates.result.resourceTemplates.map(({ uriTemplate }) => uriTemplate), ["kdlc://jobs/{job_id}","kb://{knowledge_base_id}/{concept_id}"]);
...
test("FEAT-006 served HTTP maps principals and scopes server-side before disclosure", async (t) => {
  ...
  await assert.rejects(() => createStreamableHttpServer({ server, principalMapper: mapper, host: "0.0.0.0" }), /requires injected TLS/);
  const http = await createStreamableHttpServer({ server, principalMapper: mapper }); t.after(() => http.close()); const { port } = http.address(); const endpoint = `http://127.0.0.1:${port}/mcp`; const headers = { "content-type": "application/json", accept: "application/json", "mcp-protocol-version": "2025-06-18" };
  const unauthorized = await fetch(endpoint, { method: "POST", headers, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }) }); assert.equal(unauthorized.status, 401);
  const authorized = await fetch(endpoint, { method: "POST", headers: { ...headers, authorization: "Bearer read-token" }, body: JSON.stringify({ jsonrpc: "2.0", id: 2, method: "tools/list" }) }); const payload = await authorized.json(); assert(payload.result.tools.every(({ name }) => !["ingest_start","publish_request","review_submit","job_cancel"].includes(name)));
  const denied = await fetch(endpoint, { method: "POST", headers: { ...headers, authorization: "Bearer read-token" }, body: JSON.stringify({ jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "publish_request", arguments: { principal: { actor: "human:admin" } } } }) }); assert.equal((await denied.json()).error.message, "Resource unavailable");
});
...
test("FEAT-006 transports fail closed on malformed, unauthorized, and remote-path requests", async (t) => {
  ...
  const input = new PassThrough(), output = new PassThrough(); let text = ""; output.setEncoding("utf8"); output.on("data", chunk => { text += chunk; }); const serving = serveStdio(server, { input, output }); input.end('{bad json}\n'+JSON.stringify({jsonrpc:"2.0",method:"tools/list"})+'\n'); await serving; const lines = text.trim().split("\n").map(JSON.parse); assert.equal(lines.length, 1); assert.equal(lines[0].error.code, -32700);
  const denied = await server.request({ jsonrpc:"2.0", id:1, method:"resources/list" }, { actor:"human:none", scopes:[] }); assert.equal(denied.error.code, -32004);
  const remote = new KdlcEngine({ root, clock, principal:{actor:"human:r",scopes:["mutate"],principal_mode:"served"}, handlers:{ ingest:()=>({ok:true}) }, remoteSources:{objectIds:["upload_safe"]} });
  await assert.rejects(() => remote.execute("ingest_start", { sources:["../../etc/passwd"], idempotency_key:"x" }), error => error.exitClass === EXIT.policy);
  const accepted = await remote.execute("ingest_start", { sources:["upload_safe"], idempotency_key:"y" }); assert.equal(accepted.state,"queued");
});

test("FEAT-006 CLI and doctor reject empty or corrupt inputs", async (t) => {
  assert.throws(() => parseCli(["query"]), error => error.exitClass === EXIT.input); assert.throws(() => parseCli(["ingest"]), error => error.exitClass === EXIT.input);
  const root = await temporary(t); const engine = new KdlcEngine({root,clock}); await engine.execute("init",{project_id:"fixture.project"}); await import("node:fs/promises").then(({writeFile})=>writeFile(resolve(root,"knowledge.lock"),"not-json")); const result=await engine.execute("doctor"); assert.equal(result.healthy,false); assert.equal(result.diagnostics.find(({id})=>id==="mounts.lock").status,"fail");
});
...
test("FEAT-006 MCP resources, schemas, metadata, doctor, and generated drift are accurate", async (t) => {
  ...
  assert.equal((await server.resource("kdlc://server/info")).label, "kdlc"); assert.equal((await server.resource(`kdlc://jobs/${job.id}`)).id, job.id); assert.equal((await engine.execute("doctor")).healthy, Number(process.versions.node.split(".")[0]) < 25);
  ...
  const conformance = JSON.parse(await readFile(resolve(repository, "distribution/conformance.json"), "utf8")); assert.deepEqual(conformance.tools, server.serverInfo().tools); assert.equal(conformance.repository_analysis, false);
