...
import { loadPreregistration, scoreCaptures, sha256, validateCapture, wilsonLower } from "../../scripts/statistical-evidence-validation.mjs";

const root = resolve(import.meta.dirname, "../..");
async function perfectCapture(index) {
  const state = await loadPreregistration(root);
  const results = state.documents.corpus.cases.map((entry) => {
    ...
    return { case_id: entry.id, response, raw_output, raw_output_hash: sha256(raw_output) };
  });
  return { api_version: "kdlc.dev/statistical-capture/v1alpha1", trial_id: `trial-${String(index).padStart(3, "0")}`, captured_at: "2026-08-15T12:00:00Z", corpus_hash: state.hashes.corpus, profile_hash: state.hashes.profile, manifest_hashes: state.documents.profile.manifest_hashes, provider_request_ids: results.map((_, item) => `provider-${index}-${item}`), results, exclusions: [] };
}
...
test("REL-001 capture validation rejects missing cases, substitutions, raw-output drift, exclusions, and provenance drift", async () => {
  for (const mutation of ["missing", "substitution", "raw", "exclusion", "provenance"]) {
