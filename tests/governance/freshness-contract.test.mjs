...
const retrievalPolicy = { api_version: "kdlc.dev/governance-policy/v1alpha1", id: "freshness-retrieval", version: 1, minimum_independent_sources: 1, required_erasure_surfaces: [], waiver_authorities: {}, declassification_authorities: {}, erasure_policy_refs: {}, external_models: {} };
async function governanceControls(instant) {
  const clock = { now: () => instant }; const audit = { append: async () => {} };
  const authority = new GovernanceControlAuthority({ authenticate: async () => null, clock, audit });
  return GovernanceControlEngine.create({ policy: retrievalPolicy, clock, audit, authority });
}
const reader = { actor: "human:reader", clearance: "internal", compartments: ["engineering"] };
...
test("FEAT-004 publication artifact crosses into FEAT-005 fresh retrieval at an exact date boundary", async (context) => {
  ...
  for (const [instant, mode, staleBehavior, expected, freshness, warnings] of cases) {
    const retriever = new FederatedRetriever({ mounts, policy, governanceControls: await governanceControls(instant), now: () => new Date(instant), minimumDurationMs: 0 });
    const principal = reader, authorization = await retriever.prepareAuthorization({ principal, query: "token lifetime", queryModes: [mode] });
  ...
  const invalidRetriever = new FederatedRetriever({ mounts: invalidMounts, policy, governanceControls: await governanceControls("2030-01-01T12:00:00Z"), now: () => new Date("2030-01-01T12:00:00Z"), minimumDurationMs: 0 });
  const invalidPrincipal = reader, invalidAuthorization = await invalidRetriever.prepareAuthorization({ principal: invalidPrincipal, query: "token lifetime", queryModes: ["wiki-only", "fresh-only"] });
