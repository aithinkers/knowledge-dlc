...
test("FEAT-004 FEAT-005 share strict Gregorian and RFC3339 temporal contracts", () => {
  ...
  for (const value of ["2027-02-29T00:00:00Z", "2028-02-29T24:00:00Z", "2028-02-29T00:00:00+24:00"]) assert.equal(isRfc3339Instant(value), false);
...
test("FEAT-004 publication artifact crosses into FEAT-005 fresh retrieval at an exact date boundary", async (context) => {
  ...
  for (const [instant, expected] of [["2030-01-01T23:59:59Z", "ok"], ["2030-01-02T00:00:00Z", "not_found"]]) {
    ...
    const result = await retriever.search({ authorization, principal, query: "token lifetime", mode: "fresh-only", minimumTrust: "human-reviewed", staleBehavior: "exclude" });
