...
if (derivedArtifacts.head_sha !== process.env.KDLC_HEAD_SHA || results.some(({ platform, observed_evidence }) => JSON.stringify(observed_evidence.supply_chain) !== JSON.stringify(derivedArtifacts.supply_chain) || JSON.stringify(observed_evidence.smoke) !== JSON.stringify(derivedArtifacts.smoke) || observed_evidence.package.content_sha256 !== derivedArtifacts.package.content_sha256 || observed_evidence.package.file_count !== derivedArtifacts.package.file_count || (platform.os === "linux" && JSON.stringify(observed_evidence.package) !== JSON.stringify(derivedArtifacts.package)))) throw new Error("cell evidence differs from trusted canonical content or Ubuntu release-artifact derivation");
