...
const { stdout: npmVersionOutput } = await execute(npmCommand, ["--version"]); const npmVersion = npmVersionOutput.trim();
...
const run = async (id, command, args, options = {}) => { await execute(command, args, { cwd: root, maxBuffer: 64 * 1024 * 1024, ...options }); commands.push({ id, status: "passed" }); };
try {
  ...
  const { stdout } = await execute(npmCommand, ["pack", "--json", "--ignore-scripts", "--pack-destination", temporary], { cwd: root, maxBuffer: 16 * 1024 * 1024 });
  ...
  await execute(npmCommand, ["install", "--ignore-scripts", "--no-audit", "--no-fund", resolve(temporary, pack[0].filename)], { cwd: consumer, maxBuffer: 32 * 1024 * 1024 });
