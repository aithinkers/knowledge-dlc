...
import { assertArchiveManifestMatch, inspectPackageArchive, normalizeNpmPackPath, npmCommandInvocation } from "./supply-chain-validation.mjs";
...
try {
  ...
  for (const destination of packDirectories) {
    ...
    const artifactPath = resolve(destination, parsed[0].filename); const artifact = await readFile(artifactPath); const contents = await inspectPackageArchive(artifactPath); assertArchiveManifestMatch(contents, manifest); builds.push({ filename: parsed[0].filename, sha256: digest(artifact), manifest_sha256: digest(JSON.stringify(manifest)), content_sha256: contents.content_sha256, file_count: contents.file_count });
  }
