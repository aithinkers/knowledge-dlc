...
import { loadCaptures, loadPreregistration, scoreCaptures, validatePendingStatisticalEvidence } from "./statistical-evidence-validation.mjs";
...
if (!required) {
  const failures = await validatePendingStatisticalEvidence(root); if (failures.length) throw new Error(failures.join("; "));
  console.log("REL-001 statistical preregistration verified; provider capture remains explicitly blocked at 0/30 trials.");
} else {
