...
import { assertArchiveManifestMatch, inspectPackageArchive, normalizeNpmPackPath } from "./supply-chain-validation.mjs";
...
await withReleaseCleanup(async () => {
  ...
  for (const destination of destinations) {
    ...
    const manifest = parsed[0].files.map(({ path, size, mode }) => ({ path: normalizeNpmPackPath(path), size, mode })).sort((left, right) => left.path.localeCompare(right.path, "en")); const archive = resolve(destination, parsed[0].filename); const bytes = await readFile(archive); const contents = await inspectPackageArchive(archive); assertArchiveManifestMatch(contents, manifest);
    builds.push({ filename: parsed[0].filename, sha256: digest(bytes), manifest_sha256: digest(JSON.stringify(manifest)), content_sha256: contents.content_sha256, file_count: contents.file_count });
  }
