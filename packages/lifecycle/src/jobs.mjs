...
export class JobRegistry {
  constructor({ store, clock, ids, audit }) { Object.assign(this, { store, clock, ids, audit }); this.queue = Promise.resolve(); }
  ...
  async #serialized(action) { const running = this.queue.then(action); this.queue = running.catch(() => {}); return running; }
  ...
  async requestCancellation(id, principal) {
  ...
  async cancellationPoint(id, checkpoint) {
