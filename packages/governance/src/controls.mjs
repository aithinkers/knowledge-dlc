...
function falsehoodCodes(context, policy) {
  ...
  for (const claim of claims) {
    ...
    const hashes = distinct(sources.map((source) => source.source_hash).filter(Boolean));
    ...
    if (claim.consequential === true && hashes.length < policy.minimum_independent_sources) codes.push("KDLC_FALSEHOOD_CORROBORATION_REQUIRED");
...
export class GovernanceControlAuthority {
  #authenticate; #clock; #audit; #waivers = new WeakMap(); #declassifications = new WeakMap();
  ...
  async issueWaiver(session, { id, sensor_id, gate, subject, reason, expires_at }) {
    const identity = this.#identity(session, ["security", "governance", "legal", "records"]); const now = at(this.#clock);
    if (typeof id !== "string" || !id || typeof reason !== "string" || !reason || !GATES.has(gate) || !BUILT_IN_GOVERNANCE_SENSORS.some((sensor) => sensor.id === sensor_id) || typeof subject !== "string" || !subject || !Number.isFinite(Date.parse(expires_at)) || Date.parse(expires_at) <= now.getTime()) fail("KDLC_WAIVER_INVALID", "Waiver scope and validity must be exact and current");
  ...
  async issueDeclassification(session, { id, subject, from, to, policy_ref, reason, expires_at }) {
    const identity = this.#identity(session, ["security", "governance"]); const now = at(this.#clock);
    if (!id || !subject || !classification(from) || !classification(to) || CLASSIFICATION[to] >= CLASSIFICATION[from] || !policy_ref || !reason || !Number.isFinite(Date.parse(expires_at)) || Date.parse(expires_at) <= now.getTime()) fail("KDLC_DECLASSIFICATION_INVALID", "Declassification must be a scoped, current downgrade under policy");
...
export class GovernanceControlEngine {
  ...
  constructor({ policy, clock, audit, authority, validator }) {
    ...
    this.#policy = Object.freeze(jsonClone(policy)); this.#policyHash = artifactHash(this.#policy); this.#clock = clock; this.#audit = audit; this.#authority = authority; this.#validator = validator;
  }
  ...
  #codes(sensor, context, now) {
    ...
    if (sensor.id === "classification-declassification") {
      ...
      if (derived && CLASSIFICATION[derived] < CLASSIFICATION[required]) {
        ...
        if (!auth || auth.subject !== context.subject || auth.from !== required || auth.to !== derived || Date.parse(auth.expires_at) <= now.getTime()) return ["KDLC_DECLASSIFICATION_REQUIRED"];
      }
    ...
    if (sensor.id === "retention-legal-hold") {
      ...
      if (!context.authority_authenticated) return ["KDLC_ERASURE_AUTHORITY_REQUIRED"];
      if (context.legal_hold === true) return ["KDLC_LEGAL_HOLD"];
      ...
      if (context.propagation_verified !== true) return ["KDLC_ERASURE_PROPAGATION_UNVERIFIED"];
      return [];
    }
  ...
  async evaluate(gate, input, { waivers = [] } = {}) {
    ...
    const now = at(this.#clock); const { declassification, ...serializableInput } = input;
    const context = { ...jsonClone({ ...serializableInput, gate }), declassification };
    const results = [];
...
export function propagateGovernanceMetadata({ materials, target = {} }) {
  ...
  const legalReview = rightsCodes({ materials, target, transformation: target.transformation ?? "derivative", nowMillis: 0 }).length > 0;
