...
export function createLocalProjectEngine(options = {}) {
  ...
  if (policy && existsSync(authorityPath) && Array.isArray(policy.review_contexts)) {
    ...
    if (authorityRecord.api_version === "kdlc.dev/runtime-authority/v1" && key.byteLength >= 32 && portable(authorityRecord.key_id)) {
      ...
      const materializePublication = async ({ workflowId, proposal }) => {
        ...
        // FEAT-047 (#154): hierarchical progressive-disclosure indexes — the
        // reviewed title/description of every published concept, one index
        // per directory, regenerated deterministically inside the same
        // transaction (OKF bundle browsing parity).
        const describeEntry = async (item) => {
          if (item.id === conceptId) return { path: item.path, title: frontmatter.title ?? item.id, description: frontmatter.description ?? "" };
          try {
            const text = await indexStore.readText(`${base}/${item.path}`);
            const fm = parseYaml(text.split(/^---$/m)[1] ?? "") ?? {};
            return { path: item.path, title: fm.title ?? item.id, description: fm.description ?? "" };
          } catch {
            return { path: item.path, title: item.id, description: "" };
          }
        };
        let indexFiles;
        try {
          indexFiles = generateHierarchicalIndexes(await Promise.all(catalog.concepts.map(describeEntry)));
        } catch (error) {
          throw inputError(`proposal subject ${proposal.target.subject} cannot be indexed: ${error.message}`);
        }
        ...
        return { materialized: true, concept: `${base}/${conceptRelative}`, index: indexPathRel, indexes: indexFiles.size, catalog: catalogPath, transaction_id: journal.transaction_id };
      };
  ...
  const handlers = policy
    ? {
        ...
        ingest,
        adopt: ingest,
        // FEAT-047 (#154): a self-contained, dependency-free interactive map
        // of the published knowledge base — nodes from the retrieval catalog
        // (reviewed titles/descriptions), edges from frontmatter
        // relationships. Written beside the catalog as a generated artifact.
        visualize: async () => {
          const project = JSON.parse(readFileSync(resolve(root, ".kdlc/project.json"), "utf8"));
          const mount = project.mounts?.[0]?.alias ?? "primary";
          const base = `knowledge/${mount}`;
          const catalogStore = new NodeFileStore(root);
          if (!(await catalogStore.exists(`${base}/retrieval-catalog.json`))) throw missing("No retrieval catalog yet — publish a concept first");
          const catalog = JSON.parse(await catalogStore.readText(`${base}/retrieval-catalog.json`));
...
const svg=document.querySelector("svg"),detail=document.getElementById("d");
const W=()=>svg.clientWidth,H=()=>svg.clientHeight;
const ns="http://www.w3.org/2000/svg";
const idx=new Map(DATA.nodes.map((n,i)=>[n.id,i]));
const pos=DATA.nodes.map((n,i)=>({x:W()/2+Math.cos(i*2.399)*(60+8*Math.sqrt(i))*1.6,y:H()/2+Math.sin(i*2.399)*(60+8*Math.sqrt(i)),vx:0,vy:0}));
for(let t=0;t<300;t++){for(let i=0;i<pos.length;i++)for(let j=i+1;j<pos.length;j++){const a=pos[i],b=pos[j];let dx=b.x-a.x,dy=b.y-a.y,d2=dx*dx+dy*dy+0.01,f=1200/d2;a.vx-=dx*f;a.vy-=dy*f;b.vx+=dx*f;b.vy+=dy*f;}
...
const t=document.createElementNS(ns,"text");t.setAttribute("x",pos[i].x+11);t.setAttribute("y",pos[i].y+4);t.textContent=n.title.slice(0,28);g.append(c,t);svg.appendChild(g);});
          ...
          return { visualization: `${base}/viz.html`, nodes: nodes.length, edges: edges.length, note: "self-contained HTML — open it in any browser" };
        },
        lint: async () => {
          const contracts = await createContractValidator();
          const project = parseYamlArtifact(
